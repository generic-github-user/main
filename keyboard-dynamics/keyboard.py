...
layout = [line+('?'*(max_len-len(line))) for line in layout]
...
def transition_cost(
    ...
    # Calculate the cost associated with the distance between pointers (i.e.,
    # the distance fingers would need to stretch to reach certain keys)
    ...
    if stretch_mode == 'mean':
        if groups:
            ...
            stretch_cost = np.sum(
                np.linalg.norm(new - np.mean(new, axis=0), ord=2))
    elif stretch_mode == 'all':
        stretch_cost = np.linalg.norm(new[..., None] - new.T[None, ...])
        # stretch_cost = np.linalg.norm(
        #   new[...,None] - new.T[None,...], axis=(0, 1), ord=2)
...
for method in ['random', 'nearest']:
    s2, v2 = sample_random(500, string_list=s, positioning=method,
                           stretch_penalty=0.2, **defaults)
...
# Base code for grouped bar chart from
